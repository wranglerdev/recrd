...
- **Global scope**: `/home/gil/dev/recrd/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/gil/dev/recrd/.claude/notes/`)
...
2. Glob `/home/gil/dev/recrd/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/gil/dev/recrd/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/gil/dev/recrd/.claude/notes/` — works in any directory
