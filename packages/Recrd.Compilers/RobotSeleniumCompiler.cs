...
public sealed class RobotSeleniumCompiler : ITestCompiler
{
    ...
    public async Task<CompilationResult> CompileAsync(Session session, CompilerOptions options)
    {
        ...
        {
            await writer.WriteLineAsync(HeaderEmitter.Emit("robot-selenium", options.SourceFilePath, session));
            await writer.WriteLineAsync("*** Settings ***");
            await writer.WriteLineAsync("Library    SeleniumLibrary");
            await writer.WriteLineAsync();
            await writer.WriteLineAsync("*** Keywords ***");
            await writer.WriteLineAsync("Abrir Suite");
            await writer.WriteLineAsync("    Open Browser    ${BASE_URL}    chrome");
            await writer.WriteLineAsync("    Set Selenium Implicit Wait    ${TIMEOUT}s");
            await writer.WriteLineAsync();
            await writer.WriteLineAsync("Fechar Suite");
            await writer.WriteLineAsync("    Close All Browsers");
            ...
            foreach (var (step, name) in keywordMap)
            {
                ...
                switch (step)
                {
                    case ActionStep actionStep:
                        await SeleniumKeywordEmitter.WriteKeywordAsync(writer, actionStep, options, warnings);
                        break;
                    case AssertionStep assertionStep:
                        await SeleniumKeywordEmitter.WriteAssertionKeywordAsync(writer, assertionStep, options, warnings);
                        break;
                }
            }
        }
        ...
        {
            ["robotframework"] = "7.x",
            ["robotframework-seleniumlibrary"] = "6.x"
        };
